...
// 线程池中执行的任务函数声明
void READCLIENTREQUEST(int client_fd, std::vector<char*>& buffer_vector, std::mutex* buffer_locks, Ilru_mgr<int, sockaddr_in>* client_fd_address_lru, fd_set& allfds, std::mutex& update_fd_set_mutex);
...
void Itcp_manager_implement::eventLoop(int server_fd) {
    ...
    while (server_controller.find(server_fd) != server_controller.end() && this->server_controller[server_fd] == true) {    // 检查此服务端是否还需要继续运行
...

        {
            ...
            activity = select(server_fd_max_fd[server_fd] + 1, &server_fd_set_map[server_fd].readfds, nullptr, nullptr, nullptr);
        }
        if (activity < 0) {
            ...
            for (int i = 0; i < client_size; i++) {
                ...
                if (FD_ISSET(client_fd, &server_fd_set_map[server_fd].readfds)) { // 此时clientfd客户端有新的数据到达
                    ...
                    thread_pool.enqueueTask([client_fd, this, client_fd_address_lru, &server_set, &in_process_client_fd, &in_process_mutex, &update_fd_set_mutex]() { 
                        try {
                            std::cout<<"thread pool now accept a read data task! client_fd: "<< client_fd << std::endl;
                            READCLIENTREQUEST(client_fd, this->buffer_vector, this->buffer_locks, client_fd_address_lru, server_set.allfds, update_fd_set_mutex);
                        }
                        catch (const SocketException& e) {
